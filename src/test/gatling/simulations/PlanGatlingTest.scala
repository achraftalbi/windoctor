...
class PlanGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Plan entity")
        ...
        .repeat(2) {
            ...
            .post("/api/plans")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "number":null, "archive":null, "creation_date":"2020-01-01T00:00:00.000Z"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_plan_url")))
            .pause(10)
            .repeat(5) {
