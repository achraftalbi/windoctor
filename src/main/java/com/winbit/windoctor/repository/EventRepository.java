...
public interface EventRepository extends JpaRepository<Event,Long> {
    ...
    @Query("select e from Event e where e.event_date between ?1 and ?2 and e.user.structure.id = ?3 and e.eventStatus.id!=8 order by e.event_date asc")
    ...
    @Query("select e from Event e where e.event_date between ?1 and ?2 and e.user.structure.id = ?3 order by e.event_date asc")
