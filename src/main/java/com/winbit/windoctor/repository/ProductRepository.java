...
public interface ProductRepository extends JpaRepository<Product,Long> {
    ...
    @Query("select p from Product p where p.structure.id = ?1 and (p.threshold>0 and " +
        "(coalesce((select coalesce(sum(coalesce(pu.amount,0)),0) from Purchase pu where pu.purchase_product.id=p.id),0)-" +
        "coalesce((select coalesce(sum(coalesce(co.amount,0)),0) from Consumption co where co.consumption_product.id=p.id),0)) < p.threshold)")
